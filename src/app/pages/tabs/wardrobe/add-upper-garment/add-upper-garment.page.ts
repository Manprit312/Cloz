...
@Component({
  selector: 'app-add-upper-garment',
  templateUrl: './add-upper-garment.page.html',
  styleUrls: ['./add-upper-garment.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class AddUpperGarmentPage implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    console.log('AddUpperGarmentPage - ngOnInit state:', state);
    if (state && state.garmentData) {
      const data: UpperGarmentData = state.garmentData;
      console.log('AddUpperGarmentPage - Setting edit mode with data:', data);
      this.isEditMode = true;
      this.itemId = data.id || '';
      console.log('AddUpperGarmentPage - itemId set to:', this.itemId, 'type:', typeof this.itemId, 'truthy:', !!this.itemId);
  ...
  addToWardrobe() {
    ...
    if (this.isEditMode && this.itemId && this.itemId.trim() !== '') {
      ...
      this.wardrobeService.updateItem(this.itemId, wardrobeItemData).subscribe({
        next: (response) => {
          console.log('✅ Garment updated successfully:', response);
        ...
        error: (err) => {
          ...
          this.isSubmitting = false;
          this.showErrorMessage = true;
          this.errorMessageText = err.error?.message || 'Failed to update garment. Please try again.';
          setTimeout(() => {
            this.showErrorMessage = false;
          }, 5000);
        }
    ...
    console.log('ADD MODE: Adding new garment');
    ...
    this.wardrobeService.addItem(wardrobeItemData).subscribe({
      next: (response) => {
        console.log('✅ Garment added:', response);
        this.isSubmitting = false;
        this.location.back();
      },
      error: (err) => {
        ...
        this.isSubmitting = false;
        this.showErrorMessage = true;
        this.errorMessageText = err.error?.message || 'Failed to save garment. Please try again.';
        setTimeout(() => {
          this.showErrorMessage = false;
        }, 5000);
      },
