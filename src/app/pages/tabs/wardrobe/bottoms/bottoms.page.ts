...
@Component({
  ...
  templateUrl: './bottoms.page.html',
  styleUrls: ['./bottoms.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    IonContent,
...
export class BottomsPage implements OnInit, ViewWillEnter {
  ...
  /**
   * Get available subtypes for bottoms based on user's gender
   */
  get subtypes(): string[] {
    const gender = this.profileService.profile().gender;
    return this.subtypesService.getSubtypesForCategory('bottoms', gender);
  }
  ...
  /**
   * Reload items when page becomes visible (e.g., after adding/updating/deleting a bottom)
   */
  ...
  loadBottoms(): void {
    ...
    this.wardrobeService.getBottoms().subscribe({
      next: (items) => {
        console.log('BottomsPage - Loaded items:', items);
        // Transform API items to BottomItem format
        this.items = items.map((item) => this.transformToBottomItem(item));
        this.isLoading = false;
      },
      error: (err) => {
        console.error('BottomsPage - Error loading bottoms:', err);
        this.error = err.error?.message || 'Failed to load bottoms. Please try again.';
        this.isLoading = false;
  ...
  onItemClick(item: BottomItem): void {
    this.router.navigate(['/tabs/wardrobe/bottom-detail'], {
      state: { item },
    });
  }
  ...
  hasMultipleImages(item: BottomItem): boolean {
    return this.getImages(item).length > 1;
  }

  getImages(item: BottomItem): string[] {
    const images: string[] = [];
    if (item.imageUrl) {
  ...
  getCurrentImageIndex(item: BottomItem): number {
    const index = this.itemImageIndices.get(item.id);
    return index !== undefined ? index : 0;
  }

  getCurrentImage(item: BottomItem): string {
    const images = this.getImages(item);
    const index = this.getCurrentImageIndex(item);
    return images[index] || item.imageUrl;
  }

  onTouchStart(event: TouchEvent, item: BottomItem): void {
    this.touchStartX = event.changedTouches[0].screenX;
    this.touchStartTime = Date.now();
  }

  onTouchEnd(event: TouchEvent, item: BottomItem): void {
    this.touchEndX = event.changedTouches[0].screenX;
    this.touchEndTime = Date.now();
    this.handleSwipe(item);
  }

  onCarouselClick(event: Event, item: BottomItem): void {
  ...
  private handleSwipe(item: BottomItem): void {
