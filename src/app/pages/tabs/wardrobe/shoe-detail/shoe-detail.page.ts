...
import { ShoeItem } from '../shoes/shoes.page';
...
export class ShoeDetailPage implements OnInit, ViewWillEnter {
  ...
  ionViewWillEnter(): void {
    console.log('Shoe detail page - ionViewWillEnter called, item:', this.item);
    ...
    if (this.item?.id) {
      console.log('Shoe detail page - Reloading item from API with ID:', this.item.id);
      this.reloadItemFromAPI(this.item.id);
    } else {
      console.log('Shoe detail page - No item ID found, skipping reload');
    }
  }
  ...
  private reloadItemFromAPI(itemId: string): void {
    console.log('Shoe detail page - reloadItemFromAPI called for ID:', itemId);
    // Fetch all items and find the one with matching ID
    this.wardrobeService.getShoes().subscribe({
      next: (items) => {
        console.log('Shoe detail page - Fetched items from API, count:', items.length);
        const updatedItem = items.find(item => item.id === itemId);
        if (updatedItem) {
          console.log('Shoe detail page - Found updated item:', updatedItem);
          // Transform to ShoeItem format
          const transformedItem = this.transformToShoeItem(updatedItem);
          this.item = transformedItem;
          this.brandDraft = transformedItem.brand || '';
          this.initializeImages();
          this.loadLinkedOutfits();
          console.log('Shoe detail page - ✅ Item successfully reloaded from API:', transformedItem);
        } else {
          console.warn('Shoe detail page - Item not found in API response with ID:', itemId);
        }
      },
      error: (err) => {
        console.error('Shoe detail page - ❌ Error reloading item:', err);
  ...
  private updateItemFields(update: UpdateWardrobeItemRequest): void {
    ...
    this.wardrobeService.updateItem(this.item.id, update).subscribe({
      ...
      error: (err) => {
        console.error('Shoe detail page - Error updating item:', err);
      },
    });
  }
  ...
  private loadLinkedOutfits(): void {
    ...
    this.outfitsService.getOutfits().subscribe({
      ...
      error: (err) => {
        console.error('Shoe detail page - Error loading outfits:', err);
        this.linkedOutfits = [];
        this.isLoadingOutfits = false;
      },
    });
