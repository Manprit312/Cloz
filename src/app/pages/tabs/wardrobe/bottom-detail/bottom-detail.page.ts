...
import { BottomItem } from '../bottoms/bottoms.page';
...
@Component({
  ...
  templateUrl: './bottom-detail.page.html',
  styleUrls: ['./bottom-detail.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class BottomDetailPage implements OnInit, ViewWillEnter {
  item: BottomItem | null = null;
  ...
  ngOnInit() {
    ...
    if (state && state.item) {
      ...
      this.brandDraft = this.item?.brand || '';
  ...
  ionViewWillEnter(): void {
    console.log('Bottom detail page - ionViewWillEnter called, item:', this.item);
    ...
    if (this.item?.id) {
      console.log('Bottom detail page - Reloading item from API with ID:', this.item.id);
      this.reloadItemFromAPI(this.item.id);
    } else {
      console.log('Bottom detail page - No item ID found, skipping reload');
    }
  }
  ...
  private reloadItemFromAPI(itemId: string): void {
    console.log('Bottom detail page - reloadItemFromAPI called for ID:', itemId);
    // Fetch all items and find the one with matching ID
    this.wardrobeService.getBottoms().subscribe({
      next: (items) => {
        console.log('Bottom detail page - Fetched items from API, count:', items.length);
        const updatedItem = items.find(item => item.id === itemId);
        if (updatedItem) {
          console.log('Bottom detail page - Found updated item:', updatedItem);
          // Transform to BottomItem format
          const transformedItem = this.transformToBottomItem(updatedItem);
          this.item = transformedItem;
          this.brandDraft = transformedItem.brand || '';
          this.initializeImages();
          this.loadLinkedOutfits();
          console.log('Bottom detail page - ✅ Item successfully reloaded from API:', transformedItem);
        } else {
          console.warn('Bottom detail page - Item not found in API response with ID:', itemId);
        }
      },
      error: (err) => {
        console.error('Bottom detail page - ❌ Error reloading item:', err);
  ...
  private updateItemFields(update: UpdateWardrobeItemRequest): void {
    ...
    this.wardrobeService.updateItem(this.item.id, update).subscribe({
      ...
      error: (err) => {
        console.error('Bottom detail page - Error updating item:', err);
      },
    });
  }
  ...
  private loadLinkedOutfits(): void {
    ...
    this.outfitsService.getOutfits().subscribe({
      ...
      error: (err) => {
        console.error('Bottom detail page - Error loading outfits:', err);
        this.linkedOutfits = [];
        this.isLoadingOutfits = false;
      },
    });
  ...
  onEdit(): void {
    if (this.item) {
      this.router.navigate(['/tabs/wardrobe/add-bottom'], {
  ...
  onDeleteAiImage(): void {
    ...
    this.wardrobeService.cancelGarmentImage(this.currentImage).subscribe({
      ...
      error: (err) => {
        console.error('Bottom detail page - Error deleting AI image:', err);
      },
    });
  }
