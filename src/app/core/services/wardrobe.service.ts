...
export interface UpdateWardrobeItemRequest {
  ...
  brand?: string;
  image?: File | string; // File object or base64 string
  imageId?: string; // When replacing an image, id of the image being replaced
}
...
export class WardrobeService {
   ...
   * Backend returns: { images: [{ id, imageUrl, isPrimary, ... }] }
   * Frontend expects: { imageUrl, imageUrls, imageIds } with imageIds parallel to imageUrls
   */
  ...
  updateItem(itemId: string, itemData: UpdateWardrobeItemRequest): Observable<UpdateWardrobeItemResponse> {
    ...
    if (itemData.image) {
      ...
      if (itemData.imageId) {
        formData.append('imageId', itemData.imageId);
      }
    }
  ...
  /**
   * Delete a wardrobe image by item id and image id.
   * DELETE /wardrobe/{wardrobeItemId}/image/{imageId}
   * @param itemId Wardrobe item id
   * @param imageId Image id to delete
   * @returns Observable with the response
   */
  deleteWardrobeImage(itemId: string, imageId: string): Observable<any> {
    const url = `${environment.backendBaseUrl}/wardrobe/${itemId}/image/${imageId}`;
    return this.http.delete<any>(url);
  }

  /**
   * Cancel/delete a temporary garment image that was generated by AI cleanup (legacy, by URL)
